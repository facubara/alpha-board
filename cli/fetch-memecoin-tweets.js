/**
 * Fetch unanalyzed memecoin tweets for sentiment analysis.
 *
 * Usage: node cli/fetch-memecoin-tweets.js [--limit N] [--after-id N]
...
try {
  ...
  if (afterId) {
    rows = await sql`
      SELECT
        t.id,
        t.text,
        ma.handle AS author_handle,
        ma.category AS author_category,
        t.metrics,
        t.created_at
      FROM memecoin_tweets t
      JOIN memecoin_twitter_accounts ma ON ma.id = t.account_id
      LEFT JOIN memecoin_tweet_signals s ON s.tweet_id = t.id
  ...
  console.error("Error fetching memecoin tweets:", err.message);
  process.exit(1);
}
