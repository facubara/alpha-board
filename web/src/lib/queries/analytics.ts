 ...
 */
export async function getDailyArchetypePnl(): Promise<DailyArchetypePnl[]> {
  const rows = await sql`
    SELECT
 ...
 */
export async function getArchetypeCost(): Promise<ArchetypeCost[]> {
  const rows = await sql`
    SELECT
      a.strategy_archetype,
 ...
 */
export async function getAgentDrawdowns(): Promise<AgentDrawdown[]> {
  const rows = await sql`
    SELECT
      a.id,
      a.display_name,
